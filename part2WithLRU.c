...
int addToPageTable(int page, int frame) {
  ...
  int victimFrame = -1;
  // If pageTable is full, eliminate the oldest page (FIFO) (CHANGE TO LRU)
  ...
  for (int i = 0; i < MAX_ADDRESSES; i++) {
    ...
    if (frame == -1) {
      ...
      if (frame == -1) {
        ...
        if (victimFrame != -1) {
          addToPhysicalMemoryWithVictim(
              victimFrame, addresses[i].page); // make new method that takes
                                               // victim page as an index
